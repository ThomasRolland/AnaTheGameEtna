...
public class PlayerInfinite : MonoBehaviour {
	...
	private static int			gameScore; // Just played game score
	private static int			thisTurnReturnDomino; // Domino returned in one turn score
	private static int			comboScore; // combo score per turn
	private static int			thisTurnSymetricalScore;
	private static int			totalScore; // Total games played score
	private static int			totalReturnDomino; // Total domino returned in this game, score
	private static int			symetricalScore; // Total symetrical bonus score
	private static int			totalComboScore; // Total combo score
	...
	private	static IAInfinite	enemy;

	public void Start() {
		Debug.Log (AppSupervisor.mapToLoad);
		gameScore = 0;
		thisTurnReturnDomino = 0;
		comboScore = 0;
		thisTurnSymetricalScore = 0;
		totalScore = AppSupervisor.scoreTotal;
		totalReturnDomino = 0;
		symetricalScore = 0;
		totalComboScore = 0;
		...
		enemy = gameObject.GetComponent<IAInfinite> ();
			...
			if (hit != null) {
				if (hit.name == "HelpBonus") {
					...
					if (hit.tag == "FreeBoard" && (!hasPlayed || hasPlayedBonus)
						...
						if (hasPlayed == false && selectedDomino.GetComponent<Domino> ().GetDominoType () == DominoType.Simple) {
							hasPlayed = true;
					...
					} else if (hit.tag == "UsableDomino" && !hasPlayedBonus) {
						selectedDomino.transform.localScale = selectedDomino.transform.localScale / 1.2f;
						selectedDomino = hit.gameObject;
						selectedDomino.transform.localScale = selectedDomino.transform.localScale * 1.2f;
						hasSelectedBonus = DominoType.NotBonus;
	...
	private void ChangeTurn() {
		...
		AppSupervisor.score = gameScore;
		AppSupervisor.score80 = GetDominationScore();
		AppSupervisor.scoreCombo = totalComboScore;
		AppSupervisor.scoreSymetric = symetricalScore;
		AppSupervisor.scoreReturned = totalReturnDomino;
		AppSupervisor.scoreTotal = totalScore;
		if (color == DominoColor.White) {
			AppSupervisor.infinitModeHasWon = 1000;
	...
	public static void ReturnClose(Coordinate position, DominoColor color) {
		...
		if (r > 0 && whoIsPlaying == PlayerType.Computer) {
			...
			thisTurnReturnDomino += r * 10;
			totalReturnDomino += r * 10;
	...
	public static void SymetricalSum(Coordinate position, DominoColor color) {
		...
		if (r > 0 && whoIsPlaying == PlayerType.Computer) {
			...
			thisTurnReturnDomino += r * 20;
			totalReturnDomino += r * 20;
			thisTurnSymetricalScore = r * 80;
			symetricalScore += r * 80;
			GameObject.Find("Killer_Text").GetComponent<Text>().text = GameMessagesAccessor.GetInGameIsLosingText();
		}
	}
	...
	private static void SetScore() {
		if (thisTurnReturnDomino > 10) {
			comboScore = thisTurnReturnDomino + 10;
			totalComboScore += comboScore;
			gameScore += comboScore + thisTurnReturnDomino + thisTurnSymetricalScore;
			totalScore = gameScore;
			GameObject.Find ("Score").GetComponent<Text> ().text = "Score : " + totalScore;
		} else if (thisTurnReturnDomino <= 10) {
			gameScore += thisTurnReturnDomino;
			totalScore = gameScore;
			GameObject.Find ("Score").GetComponent<Text> ().text = "Score : " + totalScore;
		}
		thisTurnReturnDomino = 0;
	...
	private static int GetDominationScore() {
		GameObject[][] fullMap = map.GetMap ();
		float whiteDominos = 0;
		float blackDominos = 0;

		for (int i = 0; i < fullMap.Length; i++) {
			for (int j = 0; j < fullMap [i].Length; j++) {
				if (fullMap[i][j].GetComponent<Domino>().GetDominoColor() == DominoColor.White) {
					whiteDominos++;
				} else {
					blackDominos++;
				}
		...
		if ((whiteDominos / (whiteDominos + blackDominos)) >= 0.8f) {
			return 200;
		}
		return 0;
	}
